...
# This script is used to implement the A* algorithm to find the path from start to goal position
# The script also generates a video of the path found by the algorithm
...
def actionnn(node,rpm1,rpm2):
    ul = 2*math.pi*rpm1/60                                                 # calculate the left wheel velocity
    ur = 2*math.pi*rpm2/60                                                 # calculate the right wheel velocity
    new_heading = (node[5] + np.rad2deg(((R/L)*(ul - ur)*t))) % 360        # get the current heading of the robot
    x_vel = (R/2)*(ur+ul)*np.cos(np.deg2rad(new_heading))                  # calculate the x velocity
    y_vel = (R/2)*(ur+ul)*np.sin(np.deg2rad(new_heading))                  # calculate the y velocity
    # print("X vel: ", x_vel)
    # print("Y vel: ", y_vel)
...
action_lists=[(0,rpm1),(rpm1,0),(rpm1,rpm1),(rpm1,rpm2),(rpm2,rpm1),(0,rpm2),(rpm2,0),(rpm2,rpm2)] # list of actions


start_time = time.time()                  # get the start time of the program
new_index = 1                             # initialize the index
...
while(open_list):
    ...
    for action_set in action_lists:                                                # loop to get the new node's coordinates, heading, total cost and cost to come
        point, new_heading, tc, c2c = actionnn(node,action_set[0],action_set[1])                                    # get the new node's coordinates, heading, total cost and cost to come
...
print("Actual goal reached :",(node[4][0]-50)*10, (node[4][1]-100)*10)          # print the actual goal position
...
cv2.imshow("canvas", canvas_flipped_uint8)                                 # show the final frame
cv2.waitKey(0)                                                             # wait for the key press
